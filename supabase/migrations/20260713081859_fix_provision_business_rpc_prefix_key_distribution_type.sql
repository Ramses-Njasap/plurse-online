...
-- This migration fixes the provision_business_transaction RPC function to handle key_distribution_type enum casting rather than using the text type, which was causing a runtime error when inserting into the access_keys table. The key_type parameter is now explicitly cast to the key_distribution_type enum type.
...
BEGIN
    ...
    -- ── STEP 3: Create the Access Key (With Explicit Enum Casting) ──
    INSERT INTO public.access_keys (
        ...
        v_access_key_id, 
        p_key_code, 
        p_key_type::key_distribution_type, -- ◄ FIXED: Cast text to your custom enum type
        p_amount, 
        p_deduct_trial_fee, 
        FALSE, 
        NULL, 
