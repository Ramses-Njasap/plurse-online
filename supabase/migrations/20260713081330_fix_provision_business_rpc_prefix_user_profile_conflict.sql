-- Migration: 20260713081330_fix_provision_business_rpc_prefix_user_profile_conflict.sql
-- This migration fixes the provision_business_transaction RPC function to handle conflicts when inserting into the user_profiles
-- The previous version of the function would fail if a database trigger had already created the user profile row, so we now use an UPSERT (ON CONFLICT DO UPDATE) to ensure the profile is updated with the wizard payload details instead of failing.
...
BEGIN
    ...
    -- ── STEP 2: Create Manager Profile (With Conflict Protection) ──
    -- If a database trigger already created this profile row, we update it with the wizard payload details.
    ...
    ON CONFLICT (id) DO UPDATE SET
        ...
        is_channel_partner = EXCLUDED.is_channel_partner;
