// InventoryFeatures.tsx — Zones 3–6
// Alternating text/image rows — consistent with site spacing patterns

import { FEATURES } from "@/data/solutions/inventory/data";

const Features = () => {
    return (
        <>
            {FEATURES.map((feature, i) => (
                <section
                    key={feature.id}
                    className="relative z-10 px-5 sm:px-8 py-24 sm:py-32"
                    style={{
                        background: i % 2 === 0 ? "var(--background)" : "var(--surface)",
                        borderTop: "1px solid var(--border)",
                    }}
                >
                    <div
                        className="max-w-6xl mx-auto flex flex-col md:grid md:items-center gap-10 md:gap-20 sm:flex-col"
                        style={{ gridTemplateColumns: "1fr 1fr" }}
                    >
                        {/* Text block */}
                        {(() => {
                            const textBlock = (
                                <div className="flex flex-col">
                                    <p
                                        className="text-[11px] font-semibold tracking-[0.2em] uppercase mb-4"
                                        style={{ color: "var(--brand)" }}
                                    >
                                        {feature.eyebrow}
                                    </p>
                                    <h2
                                        className="text-[1.75rem] sm:text-[2.25rem] font-bold tracking-[-0.025em] leading-[1.15] mb-5"
                                        style={{ color: "var(--foreground)", fontFamily: "var(--font-geist-sans)" }}
                                    >
                                        {feature.headline}
                                    </h2>
                                    <p
                                        className="text-[15px] leading-[1.75] mb-8"
                                        style={{ color: "var(--text-muted)" }}
                                    >
                                        {feature.body}
                                    </p>
                                    <div className="flex flex-col gap-3">
                                        {feature.callouts.map((c, ci) => (
                                            <div key={ci} className="flex items-start gap-3">
                                                <span
                                                    className="flex items-center justify-center rounded-full shrink-0 mt-0.5"
                                                    style={{
                                                        width: "20px",
                                                        height: "20px",
                                                        background: "var(--brand-light)",
                                                        border: "1px solid rgba(59,130,246,0.2)",
                                                    }}
                                                >
                                                    <svg width="10" height="10" viewBox="0 0 16 16" fill="none">
                                                        <path d="M3 8l4 4 6-7" stroke="var(--brand)" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" />
                                                    </svg>
                                                </span>
                                                <span className="text-[13.5px] leading-[1.6]" style={{ color: "var(--foreground)" }}>
                                                    {c}
                                                </span>
                                            </div>
                                        ))}
                                    </div>
                                </div>
                            );

                            const imageBlock = (
                                <div
                                    className="relative rounded-2xl overflow-hidden w-full"
                                    style={{
                                        aspectRatio: "4/3",
                                        boxShadow: "0 24px 64px rgba(15,23,42,0.1), 0 4px 16px rgba(59,130,246,0.06)",
                                        border: "1px solid var(--border)",
                                    }}
                                >
                                    <img
                                        src={feature.image}
                                        alt={feature.alt}
                                        className="w-full h-full object-cover"
                                    />
                                    <div
                                        className="absolute inset-0 pointer-events-none"
                                        style={{ background: "linear-gradient(135deg, rgba(59,130,246,0.05) 0%, transparent 55%)" }}
                                    />
                                </div>
                            );

                            // On mobile always stack text then image
                            // On desktop alternate based on flip
                            return (
                                <>
                                    <div className={feature.flip ? "md:order-2" : "md:order-1"}>
                                        {textBlock}
                                    </div>
                                    <div className={feature.flip ? "md:order-1" : "md:order-2"}>
                                        {imageBlock}
                                    </div>
                                </>
                            );
                        })()}
                    </div>
                </section>
            ))}
        </>
    );
}

export default Features;